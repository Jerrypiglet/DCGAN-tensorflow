...
class VirtualBatchNormalization(object):
	...
	def __init__(self, x, name, epsilon=1e-5, half=None):
		...
		self.dyn_batch_size = tf.shape(x)[0]
		needs_reshape = len(shape) != 4

		if needs_reshape:
			orig_shape = shape
			if len(shape) == 5:
				x = tf.reshape(x, tf.pack([self.dyn_batch_size, 1, shape[1]*shape[2]*shape[3], shape[4]]))
			elif len(shape) == 2:
				x = tf.reshape(x, tf.pack([self.dyn_batch_size, 1, 1, shape[1]]))
			elif len(shape) == 1:
				x = tf.reshape(x, tf.pack([self.dyn_batch_size, 1, 1, 1]))
			else:
				assert False, shape
			shape = x.get_shape().as_list()

		with tf.variable_scope(name) as scope:
			assert name.startswith("descriminator") or name.startswith("generator") or name.startswith("BN")
			self.epsilon = epsilon
			self.name = name
			if self.half is None:
				half = x
			elif self.half == 1:
				half = tf.slice(x, [0, 0, 0, 0], tf.pack([self.dyn_batch_size // 2, shape[1], shape[2], shape[3]]))
			elif self.half == 2:
				half = tf.slice(x, tf.pack([self.dyn_batch_size // 2, 0, 0, 0]), tf.pack([self.dyn_batch_size // 2, shape[1], shape[2], shape[3]]))
			else:
				assert False
			self.mean = tf.reduce_mean(half, [0, 1, 2], keep_dims=True)
			self.mean_sq = tf.reduce_mean(tf.square(half), [0, 1, 2], keep_dims=True)
			self.batch_size = self.dyn_batch_size
	...
	def __call__(self, x):
		...
		with tf.variable_scope(self.name, reuse=True) as scope:
			new_coeff = 1. / tf.to_float((self.batch_size + 1))
...
class VariationalAutoencoder(object):
	...
	def _create_loss_optimizer(self):
		...
		self.d_optim = tf.train.AdamOptimizer(2e-6, beta1=FLAGS.beta1) \
	...
	def _discriminator(self, input_tensor, trainable=True, reuse=False):
		with tf.variable_scope("discriminator") as scope:
			...
			with slim.arg_scope([slim.fully_connected], trainable=FLAGS.train_net):
				...
				dyn_batch_size = tf.shape(input_tensor)[0]
				# x_tensor = tf.reshape(input_tensor, [-1, 30, 30, 30, 1])
				noisy_x = input_tensor + tf.random_normal(tf.pack([dyn_batch_size, 30, 30, 30, 1]))
	...
	def _generator(self, input_sample, trainable=True):
		with tf.variable_scope("generator") as scope:
			...
			hidden_tensor_inv = vbn(tf.contrib.layers.fully_connected(input_sample, self.flatten_length//4, activation_fn=self.transfer_fct_conv, trainable=trainable), "generator-fc-1")
			...
			hidden_tensor_inv = vbn(tf.contrib.layers.fully_connected(hidden_tensor_inv, self.flatten_length, activation_fn=self.transfer_fct_conv, trainable=trainable), "generator-fc-2")
